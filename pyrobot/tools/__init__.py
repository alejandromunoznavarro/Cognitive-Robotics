...
class Option:
    def __init__(
        self, name, shortForm, argCount, evalFunc, defaultValue, helpText=None
    ):
        self.name = name
...
class OptionHandler:
        ...
        return (
            ":".join(
                [
                    ...
                    if x.shortForm != "" and x.argCount == 1
                ]
            )
            + ":"
            + ":".join(
                [
                    x.shortForm
                    for x in self.options
                    if x.shortForm != "" and x.argCount == 0
                ],
                "",
            )
        )
    ...
    def makeLongForm(self):
        return [x.name + "=" for x in self.options if x.argCount == 1] + [
            x.name for x in self.options if x.argCount == 0
        ]
    ...
    def help(self):
        ...
        for opt in self.options:
            print(
                "   --%-12s  -%-5s  %-24s %s"
                % (opt.name, opt.shortForm, "[" + opt.defaultValue + "]", opt.helpText)
            )
