...
class Trace:
    ...
    def readDataFile(self):
        ...
        for line in dataFile:
            ...
            if len(elements) < 3 or len(elements) > 5:
                ...
                dataList += [
                    [float(x) for x in elements[:3]] + [" ".join(elements[3:])]
                ]
    ...
    def getColor(self, label):
        if label in self.textDict:
            ...
            self.textDict[label] = next(self.colorSet)
            return self.textDict[label]
    ...
    def getSymbol(self, label):
        if label in self.symbolDict:
            ...
            self.symbolDict[label] = next(self.symbolSet)
            return self.symbolDict[label]
    ...
    def drawSymbol(self, loc, angle, label=None):
        ...
        if self.symbols:
            ...
            self.drawObj.ellipse(
                (loc[0] - 2, loc[1] - 2, loc[0] + 2, loc[1] + 2), fill=(0, 0, 0)
            )
    ...
    def makeWindow(self):
        if self.app != 0:
            ...
            self.visible = 1
        while self.window.tk.dooneevent(2):
            pass
